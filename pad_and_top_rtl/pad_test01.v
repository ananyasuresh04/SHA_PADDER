...
module sha256_padder (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_EMIT_LAST: begin
                    if (schedule_ready || (count == 5'd16)) state <= S_IDLE;
                end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || count1 == 5'd16)
        count1 <= 5'd0;
    else if (temp2)
        count1 <= count1 + 4'd1;   // automatically wraps after 15
    else
        count1 <= 4'd0;          // hold
    end

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || count == 5'd16)
        count <= 5'd0;
    else if (temp1)
        count <= count + 4'd1;   // automatically wraps after 15
    else
        count <= 4'd0;          // hold
    end
